...
const EditMembership = () => {
    ...
    const fetchMembership = async () => {
        try {
            const { data } = await axios.get(`https://helpapi.nypers.in/api/v1/get_single_membership/${id}`);
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            const res = await axios.put(`https://helpapi.nypers.in/api/v1/update_membership/${id}`, { planPrice });
