...
const Dashboard = () => {
  ...
  const handleFetchChat = async () => {
    try {
      const { data } = await axios.get('http://localhost:5654/api/v1/get-all-chat-record');
  ...
  const handleFetchRecharge = async () => {
    try {
      const { data } = await axios.get('http://localhost:5654/api/v1/total-recharge-amount');
  ...
  const handleFetchWithdraw = async () => {
    try {
      const { data } = await axios.get('http://localhost:5654/api/v1/total-withdraw-and-commission');
      setAllWithdraw({
