...
const AllRechargeCoupon = () => {
  ...
  const fetchCoupons = async () => {
    setLoading(true);
    try {
      const { data } = await axios.get('https://testapi.helpubuild.in/api/v1/all_recharge_coupon');
  ...
  const deleteCoupon = async (id) => {
    setLoading(true);
    try {
      await axios.delete(`https://testapi.helpubuild.in/api/v1/delete_recharge_coupon/${id}`);
      fetchCoupons();
  ...
  return (
    <>
      <Table
        ...
        btnText="Add Coupon"
        btnURL="/recharge_coupon/add_recharge_coupon"
        tableHeading={heading}
        tableContent={
          loading ? (
            <CTableRow>
              <CTableDataCell colSpan={4} className="text-center py-5">
                <CSpinner size="sm" className="me-2" /> Loading coupons...
              </CTableDataCell>
            </CTableRow>
          ) : (
              ...
              <CTableRow key={item._id}>
                ...
                <CTableDataCell>
                  <div className="action-parent">
                    <CNavLink href={`#/recharge_coupon/edit_recharge_coupon/${item._id}`} className="edit">
...
export default AllRechargeCoupon;
