...
const EditCommission = () => {
    ...
    const fetchCommissionDetails = async () => {
        try {
            const { data } = await axios.get(`https://testapi.helpubuild.in/api/v1/get-single-commision/${id}`);
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            const res = await axios.put(`https://testapi.helpubuild.in/api/v1/update-commission/${id}`, {
                commissionPercent
            });
