...
function AllProvider() {
    ...
    const updateIsBanned = async (id, currentStatus) => {
        ...
        try {
            ...
            const res = await axios.put(
                `https://helpapi.nypers.in/api/v1/update-provider-isbanned/${id}`,
    ...
    const handleDelete = async (id) => {
        setLoading(true);
        try {
            await axios.delete(`https://helpapi.nypers.in/api/v1/delete-consultant-permanent/${id}`);
            fetchProviders();
    ...
    const handleAccountVerification = async (status, providerId = selectedProvider) => {
        ...
        try {
            const res = await axios.put(
                `https://helpapi.nypers.in/api/v1/provider_verify/${providerId}`,
    ...
    const handleHelpuBuildVerify = async (id, currentStatus) => {
        try {
            const updatedStatus = !currentStatus;
            const res = await axios.put(
                `https://helpapi.nypers.in/api/v1/verified-provider/${id}`,
