...
const ERefund = () => {
  ...
  const fetchTerm = async (termType) => {
    ...
    setLoading(true);
    try {
      const { data } = await axios.get(`https://testapi.helpubuild.in/api/v1/single_term/${termType}`);
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await axios.put(`https://testapi.helpubuild.in/api/v1/update_term/${id}`, {
        text: currentText,
        type: typse
      });
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <h1 className="text-2xl font-bold mb-4">
        Edit Term: <span className="text-blue-600">Cancellation and refund</span>
      </h1>
