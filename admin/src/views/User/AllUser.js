...
function AllUser() {
  ...
  const handleFetchUsers = async () => {
    setLoading(true);
    try {
      const { data } = await axios.get('https://testapi.helpubuild.in/api/v1/users', {
        headers: {
  ...
  const handleUpdateActive = async (id, currentStatus) => {
    setLoading(true);
    try {
      const res = await axios.put(`https://testapi.helpubuild.in/api/v1/user-ban/${id}`, {
  ...
  const handleDeleteUser = async (id) => {
    setLoading(true);
    try {
      await axios.delete(`https://testapi.helpubuild.in/api/v1/user-delete/${id}`);
      handleFetchUsers();
