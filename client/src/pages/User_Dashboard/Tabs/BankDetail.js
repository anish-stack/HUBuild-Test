...
function BankDetail() {
    ...
    useEffect(() => {
        async function fetchBankDetails() {
            try {
                const response = await axios.get(`http://localhost:5654/api/v1/get-single-provider/${UserId}`);
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            const response = await axios.put(`http://localhost:5654/api/v1/update-bank-detail/${UserId}`, bankDetail);
            if (response.data.success) {
