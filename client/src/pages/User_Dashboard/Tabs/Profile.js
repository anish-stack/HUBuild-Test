...
const Profile = () => {
  ...
  useEffect(() => {
    const fetchExpertise = async () => {
      try {
        const { data } = await axios.get('https://helpapi.nypers.in/api/v1/all_expertise');
  ...
  const handleFetchProvider = async () => {
    ...
    try {
      const { data } = await axios.get(
        `https://helpapi.nypers.in/api/v1/get-single-provider/${UserId}`
      );
  ...
  const handleSendOtp = async () => {
    ...
    try {
      const response = await axios.post(
        `https://helpapi.nypers.in/api/v1/provider_number_change_request/${UserId}`,
        { newMobileNumber },
        {
          headers: {
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      const response = await axios.post(
        `https://helpapi.nypers.in/api/v1/verify_provider_change_number/${UserId}`,
        {
          otp,
          newMobileNumber
        },
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(
        `https://helpapi.nypers.in/api/v1/update-provider-profile/${UserId}`,
        payload,
        {
          headers: {
            'Content-Type': 'application/json',
