...
const UserDashboard = () => {
  ...
  const GetMyProfile = async () => {
    ...
    setLoading(true);
    try {
      const { data } = await axios.get(`https://helpapi.nypers.in/api/v1/get-single-provider/${providerId}`);
      console.log(data)
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await axios.post('https://helpapi.nypers.in/api/v1/addPortfolio?type=Portfolio', formData, {
        headers: {
  ...
  const handleIsDeactived = async (id, isDeactived) => {
    try {
      const res = await axios.patch(`https://helpapi.nypers.in/api/v1/update-provider-deactive-status/${id}`)
      if (res.data.success) {
  ...
  const handleCropComplete = async (blob) => {
    ...
    try {
      const res = await axios.put(`https://helpapi.nypers.in/api/v1/update_provider_profile_image/${providerId}`, formData)
      if (res.data.success) {
  ...
  const handleDeleteAccount = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const res = await axios.delete(`https://helpapi.nypers.in/api/v1/delete-provider/${id}`)
          if (res.data.success) {
            localStorage.clear()
  ...
  const handleFetchCommission = async () => {
    try {
      const { data } = await axios.get('https://helpapi.nypers.in/api/v1/get-all-commision')
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post("https://helpapi.nypers.in/api/v1/create-withdraw-request", {
  ...
  const sendOtp = async () => {
    try {
      const response = await axios.post('https://helpapi.nypers.in/api/v1/otp_send_before_update', { mobileNumber });
      if (response.data.success) {
        setOtpSent(true);
        setTimeout(() => {
  ...
  const verifyOtp = async () => {
    try {
      const response = await axios.post('https://helpapi.nypers.in/api/v1/verify_otp_before_update', { mobileNumber, otp });
      if (response.data.success) {
