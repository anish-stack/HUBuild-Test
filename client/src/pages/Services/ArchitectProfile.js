...
function ArchitectProfile() {
    ...
    const handleFetchProvider = async (providerId) => {
        ...
        try {
            ...
            const { data } = await axios.get(
                `http://localhost:5654/api/v1/get-service-by-provider/${providerId}/${selectedCategory}`
            );
    ...
    const handleFetchUser = async (providerId) => {
        ...
        try {
            // Fetch services for the selected category
            const { data } = await axios.get(
                `http://localhost:5654/api/v1/get-user-by-id/${UserData?._id}`
            );
    ...
    const fetchProfile = async (id) => {
        ...
        try {
            const { data } = await axios.get(`http://localhost:5654/api/v1/get-single-provider/${id}`);
    ...
    const handleFetchReview = async () => {
        try {
            const { data } = await axios.get(
                `http://localhost:5654/api/v1/get-review-by-providerId/${id}`
            );
