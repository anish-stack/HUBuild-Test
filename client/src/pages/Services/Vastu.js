...
function Vastu() {
  ...
  const handleFetchUser = async () => {
    try {
      ...
      const { data } = await axios.get(`https://helpapi.nypers.in/api/v1/get-single-user/${UserId}`)
  ...
  const handleFetchProvider = async () => {
    try {
      const { data } = await axios.get("https://helpapi.nypers.in/api/v1/get-all-provider")
  ...
  useEffect(() => {
    const handleFetchProviderAllService = async () => {
      try {
        setLoading(true)
        const all = await axios.get("https://helpapi.nypers.in/api/v1/get-all-provider-service")
  ...
  const handleFetchProviderService = async (providerId) => {
    ...
    try {
      ...
      const { data } = await axios.get(
        `https://helpapi.nypers.in/api/v1/get-service-by-provider/${providerId}/Residential`,
      )
  ...
  const fetchProviderData = async (id) => {
    try {
      const response = await axios.post(`https://helpapi.nypers.in/api/v1/provider_status/${id}`)
  ...
  const connectWithProviderWithCall = async () => {
    ...
    try {
      const res = await axios.post("https://helpapi.nypers.in/api/v1/create-call", {
        userId: UserData._id,
        providerId: id,
  ...
  const handleActiveTime = async (Chat, providerId) => {
    ...
    if (Chat === "Chat") {
      ...
      try {
        const res = await axios.post("https://helpapi.nypers.in/api/v1/create-chat", newForm)
  ...
  const handleMakePayment = async () => {
    ...
    try {
      ...
      const UserId = UserData?._id
      const res = await axios.post(`https://helpapi.nypers.in/api/v1/create-payment/${UserId}`, {
        price: amount,
      })
      ...
      if (order) {
        const options = {
          ...
          order_id: order?.id || "",
          callback_url: "https://helpapi.nypers.in/api/v1/verify-payment",
          prefill: {
            name: UserData?.name,
                  ...
                  <form>
                    <div className="top-bar">
                      <div className="architectur-bar">
                        <h3 className="architecture-heading">Talk To Vastu</h3>
                      </div>
                      {role === "user" ? (
                        <div className="architectur-bar">
