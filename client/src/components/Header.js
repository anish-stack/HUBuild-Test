...
const Header = () => {
  ...
  const fetchChatProverId = async () => {
    ...
    try {
      const url = UserData?.role === "provider"
        ? `http://localhost:5654/api/v1/get-chat-by-providerId/${UserData._id}`
        : `http://localhost:5654/api/v1/get-chat-by-userId/${UserData._id}`;
  ...
  const handleChatRead = async () => {
    ...
    try {
      const url = `http://localhost:5654/api/v1/mark-${UserData.role}-chats-as-read/${UserData._id}`;
      await axios.put(url);
    } catch (error) {
